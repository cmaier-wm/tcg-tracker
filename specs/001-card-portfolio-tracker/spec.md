...
- **Web Impact**: Keep browse, card detail, pricing history, and portfolio value/history aligned with the current desktop experience while treating the portfolio celebration/fireworks effect as desktop-only polish.
- **iOS Impact**: Match the same core browse, card detail, pricing history, and portfolio value/history behavior without introducing variant-specific UX.
- **Shared Backend/API Impact**: Reuse the existing catalog, pricing, and portfolio APIs as the common source of truth while allowing internal variation records to remain an implementation detail.
- **Parity Expectation**: `web + iOS`, with one explicit desktop-only exception for the web portfolio celebration/fireworks animation.
...
users cannot find cards and identify the correct card, portfolio tracking and
pricing views are not trustworthy.

**Independent Test**: Can be fully tested by opening the catalog, locating a
card, and confirming the card detail view shows its image and identifying
details without using portfolio features.
...
2. **Given** a card has an available image from the market data source,
...
1. **Given** a user is viewing a card with current market data,
   **When** the detail view loads, **Then** the system shows the latest tracked
   price and the date or time of that price update.
2. **Given** a card has multiple recorded price points, **When** the
...
3. **Given** a card has no historical data yet, **When** the user
...
1. **Given** a user is viewing a card, **When** they add it to their
...
- **FR-002**: The system MUST show identifying card details for each catalog
  ...
  similar names.
- **FR-003**: The system MUST display a card image when an image is available
  for the selected card.
- **FR-004**: The system MUST show the latest available market price for a
  selected card.
- **FR-005**: The system MUST retain time-series price snapshots for tracked
  card pricing so historical price graphs can be displayed.
- **FR-006**: The system MUST display a historical price graph for a selected
  card when more than one price snapshot exists.
- **FR-007**: The system MUST clearly indicate when current price or historical
  price data is unavailable.
- **FR-008**: Users MUST be able to add a card to their
  personal portfolio with a recorded quantity.
- **FR-009**: Users MUST be able to update or remove a portfolio holding.
- **FR-010**: The system MUST calculate and display the current estimated value
  of each portfolio holding using the latest available market price for that
  holding.
- **FR-011**: The system MUST calculate and display the portfolio's current
  total estimated value.
- **FR-012**: The system MUST retain periodic portfolio valuation snapshots so a
  historical portfolio value graph can be displayed.
- **FR-013**: The system MUST display a historical graph of total portfolio
  value when more than one portfolio valuation snapshot exists.
- **FR-014**: The web application MAY present a portfolio celebration/fireworks
  animation as desktop-only polish, but that animation is not required for iOS
  parity.
...
- **Portfolio Holding**: A user's owned quantity of a tracked card.
...
- **SC-003**: 95% of cards with at least two stored price snapshots
