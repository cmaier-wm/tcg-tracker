...
- Modified principles:
  ...
  - Placeholder principle set -> III. Verification Is Mandatory
...
Every behavior change MUST include executable verification evidence. Automated
tests are the default for logic, contracts, and regressions; when automation is
not yet practical, the tasks and quickstart artifacts MUST include explicit
manual verification steps and expected results. No implementation is complete
until the required verification has been run or the blocker is documented.
Unverified work is treated as incomplete work.
...
3. Verification tasks are defined for each story.
4. Project guidance files affected by the plan are updated or explicitly marked
   for follow-up.
